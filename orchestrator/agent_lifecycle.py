...
class AgentLifecycleManager:
    ...
    async def start_agent(self, agent_id: str) -> bool:
        """Start an agent and clear the intentionally_stopped flag."""
        agent_info = self.agents.get(agent_id)
        if not agent_info:
            logger.error(f"Agent {agent_id} not found")
            return False
        ...
        try:
            agent = agent_info["instance"]
            if hasattr(agent, "resume") and agent_info.get("is_paused"):
                await agent.resume()
            elif hasattr(agent, "start"):
                await agent.start()
            elif hasattr(agent, "initialize") and not agent_info.get("is_running"):
                await agent.initialize()

            agent_info["is_running"] = True
            agent_info["is_paused"] = False
            agent_info["intentionally_stopped"] = False
            self.agent_health[agent_id].status = "healthy"
            self.agent_health[agent_id].is_healthy = True
    ...
    async def stop_agent(self, agent_id: str, force: bool = False) -> bool:
        """Stop an agent and mark it as intentionally stopped to prevent auto-recovery."""
        agent_info = self.agents.get(agent_id)
        if not agent_info:
            logger.error(f"Agent {agent_id} not found")
            return False
    ...
    async def record_error(self, agent_id: str, error: str):
        ...
        self.agents[agent_id]["last_error"] = error
    ...
    async def get_agent_status(self, agent_id: str) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            "is_paused": agent_info.get("is_paused", False),
        ...
        while self.is_running:
            try:
                ...
                for agent_id, agent_info in self.agents.items():
                    ...
                    if time_since_heartbeat > self.heartbeat_timeout and not agent_info.get("is_paused"):
    ...
    async def _recovery_loop(self):
        """Recovery mechanism for unhealthy agents with exponential backoff."""
        while self.is_running:
            try:
                unhealthy_agents = [
                    agent_id for agent_id, health in self.agent_health.items()
                    if health.status == "unhealthy"
                    and self.agents[agent_id]["is_running"]
                    and not self.agents[agent_id].get("intentionally_stopped", False)
                ]

                for agent_id in unhealthy_agents:
                    restart_count = self.agents[agent_id].get("restart_count", 0)
                    if restart_count >= self.max_restart_attempts:
                        logger.warning(f"Skipping recovery for {agent_id}; restart limit reached")
                        continue

                    # Exponential backoff: 2^restart_count seconds, capped at 60s
                    backoff = min(60, 2 ** restart_count)
                    logger.info(f"Attempting recovery for agent {agent_id} (attempt {restart_count + 1}, backoff {backoff}s)")
                    self.agent_health[agent_id].status = "recovering"
                    self.agents[agent_id]["restart_count"] = restart_count + 1
