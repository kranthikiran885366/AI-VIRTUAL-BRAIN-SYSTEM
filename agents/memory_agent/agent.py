    ...
    async def recall(
        ...
        for r in scored_results:
            ...
            formatted.append({
                ...
                "content": mem.content,
                "importance": float(mem.metadata.importance),
                "confidence": float(mem.metadata.confidence),
                "user_id": mem.user_id,
                "agent_id": mem.agent_id,
                "conversation_id": mem.conversation_id,
                "created_at": mem.created_at.isoformat() if mem.created_at else None,
                "access_count": int(mem.access_count),
                "_relevance_score": float(r.relevance_score),
                "_explanation": str(r.explanation),
