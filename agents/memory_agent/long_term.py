...
class LongTermMemory:
    """Long-term memory component for the Memory Agent."""
    
    def __init__(self):
        """Initialize the long-term memory."""
        self.memories: List[Dict] = []
        self.memory_index: Dict[str, List[str]] = {}
        self.storage_path = settings.LONG_TERM_MEMORY_STORAGE_PATH
    ...
    async def shutdown(self):
        """Shutdown the long-term memory component."""
        logger.info("Shutting down long-term memory")
    ...
    async def add_memory(self, memory_data: Dict):
        ...
        logger.debug(f"Added memory to long-term storage: {memory['id']}")
    ...
    async def get_memory_count(self) -> int:
        """Get the number of memories in long-term storage."""
        if not self._initialized:
            raise RuntimeError("Long-term memory not initialized")
