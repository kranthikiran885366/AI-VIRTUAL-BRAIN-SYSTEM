  ...
  try {
    ...
    switch (action) {
      ...
      case "agent-status": {
        ...
        if (!agentName) return Response.json({ error: "Agent name required" }, { status: 400 })
        const agents = getAllAgents(true)
    ...
    switch (action) {
      ...
      case "store-memory": {
        ...
        const memory = await brainService.storeMemory(
          (user as any).id, content, memory_type, importance, tags
        )
      ...
      case "create-task": {
        ...
        const task = createTask((user as any).id, title, description, priority, due_date, tags)
      ...
      case "execute-agent": {
        const { agentName, agentAction, inputData } = params
        if (!agentName) return Response.json({ error: "agentName required" }, { status: 400 })
        const result = await brainService.executeAgent(agentName, agentAction || "process", inputData || {}, userId)
        if (user) {
          logAgentActivity(
            (user as any).id, agentName, agentAction || "process",
            inputData || {}, result || {}, true, undefined, undefined, undefined
          )
        }
